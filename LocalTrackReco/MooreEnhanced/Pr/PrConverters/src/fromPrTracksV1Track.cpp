...
namespace {
  ...
  void calculateQOverPForVelo( const PrTrackProxy& inTrack, LHCb::Event::v1::Track* outTr, const float pTVelo ) {
    ...
    for ( auto& aState : outTr->states() ) {
      ...
      auto                  covX = inTrack.StateCovX( index );
      auto                  covY = inTrack.StateCovY( index );
  ...
  void ConvertTrack( LHCb::Event::v1::Track* outTr, const PrTrackProxy& inTrack, const StateErrors& errors,
    ...
    using Tag = LHCb::Pr::tag_type_t<PrTracksType>;
    ...
    constexpr bool isForwardTrack = ( ConversionInfo::PrHistory == LHCb::Event::v1::Track::History::PrForward );
    ...
    if constexpr ( !std::is_same_v<PrTracksType, LHCb::Pr::Fitted::Forward::Tracks> ) {
    ...
    if constexpr ( stateLocations.size() > 0 ) {

      for ( std::size_t i = 0; i < stateLocations.size(); ++i ) {
        LHCb::State state;
        if constexpr ( stateLocations.size() == 1 && !isVeloTrack && !isForwardTrack ) {
          auto s = inTrack.template get<typename Tag::State>();
          state.setState( s.x().cast(), s.y().cast(), s.z().cast(), s.tx().cast(), s.ty().cast(), QOverP );
        } else {
          auto s = inTrack.template get<typename Tag::States>( stateLocations[i].second );
          state.setState( s.x().cast(), s.y().cast(), s.z().cast(), s.tx().cast(), s.ty().cast(), QOverP );
        }
        state.setLocation( stateLocations[i].first );
    ...
    // -- Fitted Forward tracks are based v1::Tracks from the forward, but with an updated first state.
    if constexpr ( std::is_same_v<PrTracksType, LHCb::Pr::Fitted::Forward::Tracks> )
      updateState( inTrack, outTr, LHCb::State::Location::ClosestToBeam );
  }
} // namespace
    ...
    namespace Track {
      namespace v1 {
        ...
        struct fromPrTracksV1Track
          ...
          using base_class_t =
              Algorithm::Transformer<LHCb::Event::v1::Tracks( const PrTracksType&, const VOneTracks&... )>;
        ...
        using PrDownstreamConverter =
            fromPrTracksV1Track<LHCb::Pr::Downstream::Tracks, LHCb::Pr::ConversionInfo::Downstream,
                                LHCb::Event::v1::Tracks>;
        ...
        using PrUpstreamConverter = fromPrTracksV1Track<LHCb::Pr::Upstream::Tracks, LHCb::Pr::ConversionInfo::Upstream,
                                                        LHCb::Event::v1::Tracks>;
        ...
        using PrForwardFromVeloUTConverter =
            fromPrTracksV1Track<LHCb::Pr::Long::Tracks, LHCb::Pr::ConversionInfo::ForwardFromVeloUT,
                                LHCb::Event::v1::Tracks>;
        DECLARE_COMPONENT_WITH_ID( PrForwardFromVeloUTConverter, "fromPrForwardTracksFromVeloUTV1Tracks" )
        using PrFittedForwardConverter =
            fromPrTracksV1Track<LHCb::Pr::Fitted::Forward::Tracks, LHCb::Pr::ConversionInfo::FittedForward,
                                LHCb::Event::v1::Tracks>;
        DECLARE_COMPONENT_WITH_ID( PrFittedForwardConverter, "fromPrFittedForwardTracksV1Tracks" )
        ...
        using PrVeloBackwardConverter =
            fromPrTracksV1Track<LHCb::Pr::Velo::Tracks, LHCb::Pr::ConversionInfo::VeloBackward>;
        ...
        using PrVeloMergerConverter =
            fromPrTracksV1TrackMerger<LHCb::Pr::Velo::Tracks, LHCb::Pr::ConversionInfo::VeloForward,
