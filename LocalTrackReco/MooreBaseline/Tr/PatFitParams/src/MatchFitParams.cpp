...
DECLARE_COMPONENT( MatchFitParams )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
MatchFitParams::MatchFitParams( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode MatchFitParams::execute() {
  ...
  for ( const LHCb::MCParticle* part : *partCtnr ) {
    ...
    // == Decay vertices
    const auto& vDecay = part->endVertices();
    double      zDecay = ( 0 < vDecay.size() ? vDecay[0]->position().z() : 20. * Gaudi::Units::m );
    ...
    // Get the Velo hits
    for ( LHCb::MCHits::const_iterator vHitIt = veloHits->begin(); veloHits->end() != vHitIt; vHitIt++ ) {
    ...
    for ( LHCb::MCHits::const_iterator iHitIt = itHits->begin(); itHits->end() != iHitIt; iHitIt++ ) {
      if ( ( *iHitIt )->mcParticle() == part ) { trHits.push_back( ( *iHitIt )->midPoint() ); }
    }

    // Get the TT hits
    for ( LHCb::MCHits::const_iterator iHittt = ttHits->begin(); ttHits->end() != iHittt; iHittt++ ) {
      if ( ( *iHittt )->mcParticle() == part ) { TTHits.push_back( ( *iHittt )->midPoint() ); }
    }

    // Get the OT hits
    for ( LHCb::MCHits::const_iterator oHitIt = otHits->begin(); otHits->end() != oHitIt; oHitIt++ ) {
      if ( ( *oHitIt )->mcParticle() == part ) { trHits.push_back( ( *oHitIt )->midPoint() ); }
    }
