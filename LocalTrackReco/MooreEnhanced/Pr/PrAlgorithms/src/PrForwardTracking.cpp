...
namespace LHCb::Pr::Forward {
  ...
  namespace {
    ...
    using ZoneCache       = FTZoneCache::ZoneCache;
    using PrForwardTracks = std::vector<PrForwardTrack>;
    using StereoSearch =
        ::Hough::HoughSearch<int, Hough::DEPTH, Hough::MAXCAND, Detector::FT::NFTUVLayers, Hough::NBINS>;
     ...
     */
    void initYFitParameters( XCandidate& trackCandidate, const VeloSeedExtended& veloSeed, const ZoneCache& cache ) {
      ...
      for ( auto zone : Detector::FT::xZonesLower ) {
    ...
    [[gnu::always_inline]] inline void separateSingleHitsForFit( Container& otherPlanes, XCandidate& protoCand,
      ...
      otherPlanes.clear();
      for ( auto iPlane{0}; iPlane < Detector::FT::NFTXLayers; ++iPlane ) {
    ...
    void fitLinearXProjection( XCandidate& protoCand, const PrParametersX& pars, const FT::Hits& SciFiHits,
      ...
      while ( fit ) {
        ...
        for ( auto itH = coordsToFit.begin(); itH != itEnd; ++itH ) {
          const auto d =
              SciFiHits.x( *itH ) - protoCand.x( veloSeed.betterZ[SciFiHits.planeCode( *itH )] - zReference );
    ...
    auto fillEmptyXPlanes( XCandidate& protoCand, const PrParametersX& pars, const VeloSeedExtended& veloSeed,
      ...
      if ( protoCand.nDifferentPlanes() == Detector::FT::NFTXLayers ) return false;
    ...
    auto fillEmptyXPlanes( PrForwardTrack& track, const PrParametersX& pars, const VeloSeedExtended& veloSeed,
      ...
      if ( track.size() == Detector::FT::NFTLayers ) return false;
      auto&                                 coordsToFit = track.getCoordsToFit();
      std::bitset<Detector::FT::NFTXLayers> hasXLayer{};
      auto                                  nX{0};
      for ( auto iHit : coordsToFit ) {
      ...
      if ( nX == Detector::FT::NFTXLayers ) return false;
      bool       added{false};
    ...
    auto collectStereoHits( StereoSearch::result_type& search_result, const PrForwardTrack& track,
      ...
      StereoSearch hough{Detector::FT::NFTUVLayers / 2, minTol, pars.uvSearchBinWidth};
      ...
      for ( auto zoneNumber : uvZones ) {
        ...
        const auto iUVStart   = SciFiHits.get_lower_bound_fast<4>( uvStart, uvEnd, xMin );
        const auto signedPred = xPredShifted * dxDySign;
        for ( auto iUV{iUVStart}; SciFiHits.x( iUV ) <= xMax; ++iUV ) {
          const auto signedDx = SciFiHits.x( iUV ) * dxDySign - signedPred;
    ...
    auto fillEmptyStereoPlanes( StereoCandidate& track, const FT::Hits& SciFiHits, const VeloSeedExtended& veloSeed,
      ...
      if ( track.nDifferentPlanes() == Detector::FT::NFTUVLayers ) return false;
      bool added{false};
      for ( auto iPlane{0}; iPlane < Detector::FT::NFTUVLayers; ++iPlane ) {
    ...
    auto selectStereoHits( PrForwardTrack& track, const FT::Hits& SciFiHits, const PrParametersY& yPars,
      ...
      static_vector<int, Detector::FT::NFTUVLayers> bestStereoHits{};
  ...
  class PrForwardTracking
      : public LHCb::Algorithm::Transformer<TracksFT( const FT::Hits&, const TrackType&, const ZoneCache&,
                                            ...
                                            LHCb::DetDesc::usesConditions<ZoneCache, DeMagnet>> {
  public:
    ...
    void selectXCandidates( PrForwardTracks&, const VeloSeedExtended&, ModSciFiHits::ModPrSciFiHitsSOA&,
                            const FT::Hits&, const PrParametersX&, const ZoneCache& ) const;
  ...
  TracksFT PrForwardTracking<TrackType>::operator()( const FT::Hits& SciFiHits, const TrackType& input_tracks,
    ...
    assert( [&] {
      ...
      for ( auto iZone{0}; iZone < Detector::FT::NFTZones; ++iZone ) {
  ...
  auto PrForwardTracking<TrackType>::forwardTracks( const TrackType& input_tracks, const FT::Hits& SciFiHits,
    ...
    for ( const auto& tr : inputTrks ) {
      ...
      if ( const float yLastStation = seed.y( cache.zoneZPos.back() );
        ...
        direct_debug( "yLastStation =", yLastStation, "not in last layer  ... skip" );
        continue;
      }
      ++acceptedInputTracks;
      ...
      assert( [&] {
        ...
        while ( coordIt != end ) {
          ...
          // long the difference is negligible w.r.t to the machine precision (epsilon).
          if ( diff <= std::numeric_limits<decltype( c1 )>::epsilon() * std::abs( c1 + c2 ) ) {
  ...
  auto PrForwardTracking<TrackType>::removeDuplicates( PrForwardTracks& tracks, const FT::Hits& SciFiHits ) const {
    ...
    auto                                                              nDuplicates{0};
    std::vector<static_vector<LHCb::LHCbID, Detector::FT::NFTLayers>> idSets( nTotal );
  ...
  auto PrForwardTracking<TrackType>::makeLHCbLongTracks( PrForwardTracks&& tracks, Container&& idSets,
    ...
    for ( auto&& [cand, ids] : Gaudi::Functional::details::zip::range( tracks, idSets ) ) {
      ...
      out.field<TracksTag::States>( 0 ).setQOverP( qOverP );
      out.field<TracksTag::States>( 1 ).setQOverP( qOverP );
      ...
      out.field<TracksTag::States>( 1 ).setPosition( pos );
      out.field<TracksTag::States>( 1 ).setDirection( dir );
      ...
      out.field<TracksTag::States>( 0 ).setPosition( ancestpos );
      out.field<TracksTag::States>( 0 ).setDirection( ancestdir );
