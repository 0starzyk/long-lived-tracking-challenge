...
class ExtrapolatorTester
    : public LHCb::Algorithm::Consumer<void( DetectorElement const& ), LHCb::DetDesc::usesConditions<DetectorElement>> {
    ...
    for ( int itx = 0; itx < nbins; ++itx )
      for ( int ity = 0; ity < nbins; ++ity ) {
        ...
        toStream( always() << prefix << "    ", origin )
            << " with q*p = "
            << ( qop != 0 ? 1. / ( Gaudi::Units::GeV * qop ) : std::numeric_limits<double>::infinity() )
