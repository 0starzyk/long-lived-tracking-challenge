...
// Constructor
StateTraj::StateTraj( const State& state, const Gaudi::XYZVector& bField )
...
// Constructor
StateTraj::StateTraj( const LHCb::StateVector& stateVector, const Gaudi::XYZVector& bField )
    : DifTraj<kSize>( 10. * Units::km, 10. * Units::km )
...
// Constructor
StateTraj::StateTraj( const Gaudi::TrackVector& stateVector, double z, const Gaudi::XYZVector& bField )
    : DifTraj<kSize>( 10. * Units::km, 10. * Units::km )
...
std::unique_ptr<Trajectory<double>> StateTraj::clone() const { return std::make_unique<StateTraj>( *this ); }
...
XYZVector StateTraj::curvature( double /*arclength*/ ) const { return m_curv; }

void StateTraj::expansion( double arclength, Gaudi::XYZPoint& p, Gaudi::XYZVector& dp, Gaudi::XYZVector& ddp ) const {
...
StateTraj::Parameters StateTraj::parameters() const {
  return Parameters( m_pos.X(), m_pos.Y(), m_dir.X() / m_dir.Z(), m_dir.Y() / m_dir.Z(), m_qOverP );
}
...
StateTraj::Derivative StateTraj::derivative( double arclength ) const {
  ...
  deriv( 2, 2 ) = -vx * vz * svz + cvz * ( vx * vy * m_bField.x() + arcvx * m_bField.y() );
  deriv( 2, 3 ) = -vy * vz * svz - cvz * ( vx * vy * m_bField.y() + arcvy * m_bField.x() );
...
// Not yet implemented
double StateTraj::distTo1stError( double, double, int ) const { return 10. * Units::km; }

// Not yet implemented
double StateTraj::distTo2ndError( double, double, int ) const { return 10. * Units::km; }
