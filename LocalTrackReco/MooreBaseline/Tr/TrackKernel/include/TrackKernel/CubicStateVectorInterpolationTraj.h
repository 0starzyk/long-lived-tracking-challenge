...
namespace LHCb {
   ...
   *
   */
  class CubicStateVectorInterpolationTraj : public ZTrajectory<double> {
  public:
    ...
    CubicStateVectorInterpolationTraj() : ZTrajectory<double>() {
    ...
    CubicStateVectorInterpolationTraj( const StateT& begin, const StateT& end ) : ZTrajectory<double>() {
      init( begin, end );
    }

    /// Constructor from single state(vector) and B-field
    template <class StateT>
    CubicStateVectorInterpolationTraj( const StateT& state, const Gaudi::XYZVector& bfield ) : ZTrajectory<double>() {
      init( state, bfield );
    }
  ...
  public: // trajectory interface
    ...
    std::unique_ptr<Trajectory<double>> clone() const override {
      return std::make_unique<CubicStateVectorInterpolationTraj>( *this );
    }
    /// Position at location z
    Point position( double z ) const override final { return Point( x( z ), y( z ), z ); }
    /// First derivative of position to z
    Vector direction( double z ) const override final { return Vector( tx( z ), ty( z ), 1 ); }
    /// Second derivative of position to z
    Vector curvature( double z ) const override final { return Vector( omegax( z ), omegay( z ), 0 ); }
    /// Distance in z until the deviation from the linear
    /// approximation differs from this trajectory by a given tolerance.
    double distTo1stError( double z, double tolerance, int pathDirection = +1 ) const override final;
    /// Distance in z until the deviation from the quadratic
    /// approximation differs from this trajectory by a given tolerance.
    double distTo2ndError( double z, double tolerance, int pathDirection = +1 ) const override final;
    ...
    double muEstimate( const Point& p ) const override final;
  ...
  public: // other public methods
    ...
    Gaudi::XYZVector momentum( double z ) const {
      return std::abs( 1 / qop( z ) ) * Gaudi::XYZVector( tx( z ), ty( z ), 1 ).Unit();
    }
    /// state vector
    StateVector stateVector( double z ) const override final;

  private:
    void   interpolate1D( double za, double xa, double txa, double zb, double xb, double txb, double coeff[4] ) const;
  ...
  template <class StateT>
  void CubicStateVectorInterpolationTraj::init( const StateT& begin, const StateT& end ) {
  ...
  template <class StateT>
  void CubicStateVectorInterpolationTraj::init( const StateT& state, const Gaudi::XYZVector& bfield ) {
...
#  pragma warning( push )
  ...
  inline double CubicStateVectorInterpolationTraj::distTo1stError( double z, double tolerance,
  ...
  inline double CubicStateVectorInterpolationTraj::distTo2ndError( double /*z*/, double tolerance,
                                                                   int /*pathDirection*/ ) const {
...
#  pragma warning( pop )
  ...
  inline double CubicStateVectorInterpolationTraj::qop( double z ) const {
    return z <= zbegin()
               ? m_qopbegin
               : ( z >= zend() ? m_qopend
                               : m_qopbegin + ( m_qopend - m_qopbegin ) * ( z - zbegin() ) / ( zend() - zbegin() ) );
  }
  ...
  inline void CubicStateVectorInterpolationTraj::interpolate1D( double za, double xa, double txa, double zb, double xb,
                                                                double txb, double c[4] ) const {
    c[0]     = xa;
    c[1]     = txa;
    double d = zb - za;
