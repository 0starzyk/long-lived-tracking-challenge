...
namespace LHCb::Pr {
  namespace {
    ...
    using SearchWindowsX  = std::array<SearchWindow, LHCb::Detector::FT::Numbers::NFTXLayers>;
    using SearchWindowUV  = std::array<std::array<SearchWindow, 2>,
                                      LHCb::Detector::FT::Numbers::NFTStations>; // BoundariesUV[U/V][T1,T2,T3][part]
    using SearchWindowsUV = std::array<SearchWindowUV, 2>;                        // BoundariesUV[U/V][T1,T2,T3][part]

    using ZoneLimitsX = std::array<ZoneLimits, LHCb::Detector::FT::Numbers::NFTXLayers>;
    using ZoneLimitUV =
        std::array<std::array<ZoneLimits, 2>, LHCb::Detector::FT::Numbers::NFTLayers>; // BoundariesUV[U/V][T1,T2,T3]
    ...
    using TrackToRecover  = std::array<std::vector<Hybrid::SeedTrackX>, nParts>;
    ...
    struct CommonXYHits {
      constexpr unsigned int operator()( int s1, int s2 ) {
        constexpr auto nCommonHits = std::array{7u, 6u, 6u, 5u, 4u}; // 70%*(12-nHits)
        auto           idx         = LHCb::Detector::FT::Numbers::NFTLayers - std::min( s1, s2 );
        assert( idx < static_cast<int>( nCommonHits.size() ) && idx >= 0 );
    ...
    struct CommonXHits {
      constexpr unsigned int operator()( int s1, int s2 ) {
        ...
        auto           idx         = 2 * LHCb::Detector::FT::Numbers::NFTXLayers - s1 - s2;
        assert( idx < static_cast<int>( nCommonHits.size() ) && idx >= 0 );
        return nCommonHits[idx];
      }
    };
    ...
    struct CommonRecoverHits {
      constexpr unsigned int operator()( int s1, int s2 ) {
        ...
        auto           idx         = LHCb::Detector::FT::Numbers::NFTXLayers - std::min( s1, s2 );
        assert( idx < static_cast<int>( nCommonHits.size() ) && idx >= 0 );
        return nCommonHits[idx];
      }
    };
    ...
    struct CaseGeomInfoXZ {
      float                                                             invZf{};
      float                                                             invZlZf{};
      std::array<unsigned int, LHCb::Detector::FT::Numbers::NFTXLayers> zones{{}};
      std::array<float, LHCb::Detector::FT::Numbers::NFTXLayers>        zLays{{}};
      std::array<float, LHCb::Detector::FT::Numbers::NFTXLayers>        dzLays{{}};
      std::array<float, LHCb::Detector::FT::Numbers::NFTXLayers>        dz2Lays{{}};
      float                                                             t2Dist;
      //  float                                                   xInfCorr{};
      float txCorr{};
      float xRefCorr{};
    ...
    void updateSmallDexes( SmallDexes& smallDexes, const PrFTHitHandler<ModPrHit>& hits ) noexcept {
      // reset values
      size_t iOffset( 0 );
      for ( int iZone = 0; iZone < LHCb::Detector::FT::NFTZones; iZone++ ) {
  ...
  private:
     ...
     */
    [[gnu::always_inline]] inline void fillXhits0( const FT::Hits& sciFiHits, unsigned int iCase,
                                                   ModPrHitConstIter Fhit, const ModPrHit& Phit, ModPrHitConstIter Lhit,
                                                   const CaseGeomInfoXZ& xZones, const TwoHitCombination& hitComb,
                                                   const ZoneLimitsX&                   zoneLimits,
                                                   std::vector<Pr::Hybrid::SeedTrackX>& xCandidates,
                                                   SearchWindowsX&                      searchWindows ) const noexcept;

    /** @brief Fills hits from the second parabola layer
     */
    [[gnu::always_inline]] inline void fillXhits1( const FT::Hits& sciFiHits, unsigned int iCase,
                                                   ModPrHitConstIter Fhit, const ModPrHit& Phit, ModPrHitConstIter Lhit,
                                                   const CaseGeomInfoXZ& xZones, const TwoHitCombination& hitComb,
                                                   const ZoneLimitsX&                   zoneLimits,
                                                   std::vector<Pr::Hybrid::SeedTrackX>& xCandidates,
    ...
    SeedTrackHitsConstIter removeWorstX( const Pr::Hybrid::SeedTrackX& track, int iCase ) const noexcept;
  ...
  private:
    ...
    Gaudi::Property<std::array<float, 3>> m_zOutputs{
  ...
  protected:
    using HoughSearch           = Hough::HoughSearch<ModPrHit const*, 2, 2, LHCb::Detector::FT::NFTUVLayers, 100>;
    ...
    public:
      ...
      template <int C = CASE>
      addStereo( const HybridSeeding& hybridSeeding, TrackToRecover& trackToRecover,
    ...
    private:
      ...
      inline HoughCandidateHitIter removeWorstY( const PrTrackFitterYZ& fitter, HoughCandidateHitIter& hitBegin,
                                                 HoughCandidateHitIter& hitEnd ) const noexcept;

      inline SeedTrackHitsConstIter removeWorstXY( const Pr::Hybrid::SeedTrack& track ) const noexcept;
  ...
  protected:
    addStereoBase( TrackToRecover& trackToRecover ) : m_trackToRecover( trackToRecover ) {}
  ...
  protected:
    TrackToRecover& m_trackToRecover;
  };

  // Clone removal namespace
  namespace {
    template <typename T>
    bool areClones( const T& tr1, const T& tr2, unsigned int maxCommon ) {
      ...
      while ( nCommon != 0 && itH1 != itEnd1 ) {
        if ( std::any_of( itBeg2, itEnd2, [id = itH1->fullDex]( const auto& h ) { return id == h.fullDex; } ) )
          --nCommon;
        ++itH1;
      }
      return ( nCommon == 0 );
    ...
    */
     ...
     */
    bool CloseEnough( const Pr::Hybrid::AbsSeedTrack& tr1, const Pr::Hybrid::AbsSeedTrack& tr2, float distance ) {
    ...
    auto removeClonesX = removeClonesT<CommonXHits, XCandidates>();
    auto removeClones  = removeClonesT<CommonXYHits, TrackCandidates>();

    // Remove tracks to recover
    void removeClonesRecover( unsigned int part, TrackToRecover& trackToRecover, XCandidates& xCandidates,
                              CommonRecoverHits commonHits = CommonRecoverHits() ) {
  ...
  Tracks HybridSeeding::operator()( const FT::Hits& hits, ZoneCache const& zoneCache, DeMagnet const& magnet ) const {
    ...
    trackToRecover[0].reserve( m_nXTrackCandidates );
    trackToRecover[1].reserve( m_nXTrackCandidates );
    ...
    if ( Recover ) {
      ...
      RecoverTrack( hits, zoneCache, oriSmallDexes, oriHitHandler, trackCandidates, xCandidates, trackToRecover );
  ...
  template <int CASE>
  template <int C>
  HybridSeeding::addStereo<CASE>::addStereo( const HybridSeeding& hybridSeeding, TrackToRecover& trackToRecover,
                                             typename std::enable_if<C != RecoverCase>::type* )
  ...
  HybridSeeding::HoughSearch HybridSeeding::addStereo<CASE>::initializeHoughSearch(
    ...
    auto calculate_factor = [zoneCache, shift = 0.f,
                             partSign = float( 1 - static_cast<int>( part ) * 2 )]( int layer ) -> float {
      return partSign / ( ( zoneCache.zoneZPos[layer] - shift ) * zoneCache.zoneDxDy[layer] );
    };
  ...
  void HybridSeeding::addStereo<CASE>::CollectLayerUV( const ZoneCache& zoneCache, unsigned int part,
                                                       unsigned int layer, const Pr::Hybrid::SeedTrackX& xProje,
    ...
    float dxDy  = zoneCache.zoneDxDy[layer]; // 0 if U, 1 if V
    float xPred = xProje.xFromDz( zoneCache.zoneZPos[layer] - Pr::Hybrid::zReference );
    ...
    for ( searchWindows[iStation][0].end = searchWindows[iStation][0].begin;
      ...
      AddUVHit( iLayer, searchWindows[iStation][0].end, xPred, hough );
    }
    // Part 1
    ...
    for ( searchWindows[iStation][1].end = searchWindows[iStation][1].begin;
      ...
      AddUVHit( iLayer, searchWindows[iStation][1].end, xPred, hough );
    }
  }
  ...
  HybridSeeding::addStereo<CASE>::removeWorstY( const PrTrackFitterYZ& fitterYZ, HoughCandidateHitIter& hitBegin,
                                                HoughCandidateHitIter& hitEnd ) const noexcept {
    const auto worst_it =
  ...
  template <int CASE>
  SeedTrackHitsConstIter HybridSeeding::addStereo<CASE>::removeWorstXY( const Pr::Hybrid::SeedTrack& track ) const
      noexcept {
      ...
      for ( auto&& itT1 : trackToRecover[part] ) {
        ...
        if ( nUsedHits < nUsed_threshold ) {
          if ( nUsedHits > 0 ) {
            ...
            if ( fitX ) {
              auto worstHitX = removeWorstX( itT1, nParts );
  ...
  void HybridSeeding::makeLHCbTracks( const FT::Hits& sciFiHits, Tracks& result, unsigned int part,
    ...
    for ( const auto& track : trackCandidates[part] ) {
      ...
      State       stateZref( Gaudi::TrackVector( track.ax(), track.yRef(), track.xSlope0(), //---LoH: can be simplified
                                                                                      // into
                                                                                      // ax(), ay()
                                           track.ySlope(), 0.f ),
                       Pr::Hybrid::zReference, State::Location::AtT );
      ...
      for ( const float z : m_zOutputs ) {
        auto state = outTrack.field<SeedTag::States>( iState );
  ...
  void HybridSeeding::findXProjections( const FT::Hits& sciFiHits, const ZoneCache& zoneCache, unsigned int part,
    ...
    for ( unsigned int i = 0; i < LHCb::Detector::FT::Numbers::NFTXLayers; ++i ) {
      float lastHitX( -5000.f );
  ...
  void HybridSeeding::createXTrack( const FT::Hits& sciFiHits, unsigned int iCase,
    ...
    bool canRemoveHitX = xCand.size() == LHCb::Detector::FT::Numbers::NFTXLayers;
  ...
  SeedTrackHitsConstIter HybridSeeding::removeWorstX( const Pr::Hybrid::SeedTrackX& track, int iCase ) const noexcept {
    const auto worst_it  = std::max_element( track.chi2Hits().cbegin(), track.chi2Hits().cbegin() + track.size() );
    auto       worst_pos = worst_it - track.chi2Hits().cbegin();
  ...
  CaseGeomInfoXZ HybridSeeding::initializeXProjections( ZoneCache const& zoneCache, unsigned int iCase,
    ...
    xZones.zLays[0]   = zoneCache.zoneZPos[firstZoneId];
    ...
    xZones.zLays[1]   = zoneCache.zoneZPos[lastZoneId];
    ...
    xZones.zLays[2]   = zoneCache.zoneZPos[LHCb::Detector::FT::UpperZones::T2X1 - part];
    ...
    xZones.zLays[3]   = zoneCache.zoneZPos[LHCb::Detector::FT::UpperZones::T2X2 - part];
    ...
    for ( unsigned int xZoneId : {LHCb::Detector::FT::UpperZones::T1X1, LHCb::Detector::FT::UpperZones::T1X2,
      ...
      if ( xZoneId != firstZoneId && xZoneId != lastZoneId ) {
        ...
        xZones.zLays[i]   = zoneCache.zoneZPos[xZoneId];
