...
DECLARE_COMPONENT( TrackGhostClassificationBase )

using namespace LHCb;

TrackGhostClassificationBase::TrackGhostClassificationBase( const std::string& type, const std::string& name,
...
StatusCode TrackGhostClassificationBase::info( const LHCb::Track& aTrack, LHCb::GhostTrackInfo& tinfo ) const {
  StatusCode sc = generic( aTrack, tinfo );
  if ( sc.isFailure() ) return sc;
  return specific( aTrack, tinfo );
}

StatusCode TrackGhostClassificationBase::generic( const LHCb::Track& aTrack, LHCb::GhostTrackInfo& tinfo ) const {
...
StatusCode TrackGhostClassificationBase::specific( const LHCb::Track& aTrack, LHCb::GhostTrackInfo& tinfo ) const {

  // default behaviour call the id version
  if ( tinfo.classification() == LHCb::GhostTrackInfo::Classification::None ) {
...
StatusCode TrackGhostClassificationBase::info( LHCbIDs::const_iterator& start, LHCbIDs::const_iterator& stop,
...
StatusCode TrackGhostClassificationBase::generic( LHCbIDs::const_iterator& start, LHCbIDs::const_iterator& stop,
                                                  LHCb::GhostTrackInfo& tinfo ) const {

  // get the information on the track
  ILHCbIDsToMCParticles::LinkMap testMap;
  StatusCode                     sc = linkTool()->link( start, stop, testMap );
  if ( sc.isFailure() ) { return Warning( "Linking failed: no classification possible", StatusCode::FAILURE, 30 ); }
  ...
  if ( !isGhost( start, stop ) ) {
    // duh its not a ghost
    tinfo.setClassification( GhostTrackInfo::Classification::Real );
    return StatusCode::SUCCESS;
  }

  // spillover
  if ( spillover( bestPair ) == true ) {
    tinfo.setClassification( GhostTrackInfo::Classification::SpilloverAndNoise );
    return StatusCode::SUCCESS;
  }
  ...
  if ( decayInFlight( bestPair, secondBest ) == true ) {
    tinfo.setClassification( GhostTrackInfo::Classification::DecayInFlight );
    return StatusCode::SUCCESS;
  }

  // conversion
  if ( conversion( bestPair, secondBest ) == true ) {
    tinfo.setClassification( GhostTrackInfo::Classification::Conversion );
    return StatusCode::SUCCESS;
  }

  // electromagnetic shower
  if ( emShower( tinfo ) == true ) {
    tinfo.setClassification( GhostTrackInfo::Classification::EM );
    return StatusCode::SUCCESS;
  }

  // interaction
  if ( interaction( tinfo ) == true ) {
    tinfo.setClassification( GhostTrackInfo::Classification::HadronicInteraction );
    return StatusCode::SUCCESS;
  }

  // phi...
  if ( fromPhi( bestPair, secondBest ) == true ) {
    tinfo.setClassification( GhostTrackInfo::Classification::FromPhi );
    return StatusCode::SUCCESS;
  }
...
StatusCode TrackGhostClassificationBase::specific( LHCbIDs::const_iterator&, LHCbIDs::const_iterator&,
...
LHCb::GhostTrackInfo::LinkPair TrackGhostClassificationBase::secondBest( const LHCb::GhostTrackInfo& tinfo,
  ...
  for ( ; iterMap != tinfo.linkMap().end(); ++iterMap ) {
    if ( iterMap->second > best.second && iterMap->first != bestPart ) { best = *iterMap; }
    tot += iterMap->second;
  } // iterMap
...
bool TrackGhostClassificationBase::decayInFlight( LHCb::GhostTrackInfo::LinkPair& p1,
  ...
  if ( p1.second + p2.second > purityCut() && p1.first != 0 && p2.first != 0 ) {
    MCParticle const* daughter = nullptr;
    MCParticle const* mother   = nullptr;
    ...
    if ( daughter && daughter->particleID().abspid() == 13 &&
         ( mother->particleID().abspid() == 211 || mother->particleID().abspid() == 321 ) )
      decay = true;
  }
  return decay;
}
...
bool TrackGhostClassificationBase::conversion( LHCb::GhostTrackInfo::LinkPair& p1,
  ...
  if ( p1.first != 0 && p2.first != 0 && ( p1.second + p2.second > purityCut() ) &&
       ...
       p1.first->mother()->particleID().abspid() == 22 )
    con = true;
...
bool TrackGhostClassificationBase::fromPhi( LHCb::GhostTrackInfo::LinkPair& p1,
  ...
  const MCParticle* m1 = 0;
  const MCParticle* m2 = 0;
...
bool TrackGhostClassificationBase::interaction( const LHCb::GhostTrackInfo& tinfo ) const {
  ...
  for ( ; iter != tinfo.linkMap().end(); ++iter ) {
    ...
    if ( origin != 0 && origin->type() == MCVertex::MCVertexType::HadronicInteraction ) { ++nHadronic; }
  }
...
bool TrackGhostClassificationBase::emShower( const LHCb::GhostTrackInfo& tinfo ) const {
  ...
  double                                        emComponent = 0.0;
  double                                        tot         = 0.0;
  LHCb::GhostTrackInfo::LinkMap::const_iterator iter        = tinfo.linkMap().begin();
  for ( ; iter != tinfo.linkMap().end(); ++iter ) {
    if ( iter->first != 0 && iter->first->particleID().abspid() == 11 ) { emComponent += iter->second; }
    tot += iter->second;
  } // iter
...
LHCb::GhostTrackInfo::LinkPair
TrackGhostClassificationBase::bestPair( const LHCb::GhostTrackInfo::LinkMap& lMap ) const {

  if ( !( lMap.size() ) ) return LHCb::GhostTrackInfo::LinkPair();
  LHCb::GhostTrackInfo::LinkMap::const_iterator    iterMap = lMap.begin();
  double                                           tot     = 0.0;
  std::pair<LHCb::MCParticle const*, unsigned int> best( nullptr, 0 );
  for ( ; iterMap != lMap.end(); ++iterMap ) {
    if ( iterMap->second > best.second ) { best = *iterMap; }
    tot += iterMap->second;
  } // iterMap
...
bool TrackGhostClassificationBase::isGhost( LHCbIDs::const_iterator& start, LHCbIDs::const_iterator& stop ) const {

  // get the information on the track
  ILHCbIDsToMCParticles::LinkMap testMap;
  linkTool()->link( start, stop, testMap ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
