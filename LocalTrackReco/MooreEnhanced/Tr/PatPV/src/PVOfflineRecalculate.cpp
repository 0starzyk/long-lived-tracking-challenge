...
DECLARE_COMPONENT( PVOfflineRecalculate )
...
//=============================================================================
PVOfflineRecalculate::PVOfflineRecalculate( const std::string& type, const std::string& name, const IInterface* parent )
    : GaudiTool( type, name, parent ) {
  declareInterface<PVOfflineRecalculate>( this );
}

StatusCode PVOfflineRecalculate::finalize() {
  if ( m_counter_count[0] > 0 ) print_stats();
...
StatusCode PVOfflineRecalculate::RecalculateVertex( const LHCb::RecVertex*                 pvin,
  ...
  for ( const LHCb::Track* itr : tracks2remove ) {
  ...
  for ( unsigned int it = 0; it < trin.size(); it++ ) {
    ...
    bool removed = remove_track( pvin, hess, d0vec, dchi2, trin[it], wgin[it], geometry );
    if ( removed ) {
  ...
  if ( msgLevel( MSG::DEBUG ) ) {
    debug() << "PV recalculate - displacement found: " << delta[0] << " , " << delta[1] << " , " << delta[2] << " , "
            << endmsg;
  }
  Gaudi::XYZPoint newVtx( pvin->position().x() + delta[0], pvin->position().y() + delta[1],
                          pvin->position().z() + delta[2] );
  vtx.setPosition( newVtx );
  if ( msgLevel( MSG::DEBUG ) ) {
    debug() << " PV recalculate - new vertex position: " << vtx.position().x() << " , " << vtx.position().y() << " , "
  ...
  if ( m_updatePVTracks ) {
    ...
    vtx.clearTracks();
    for ( const LHCb::Track* ptr : pvin->tracks() ) {
...
bool PVOfflineRecalculate::remove_track( const LHCb::RecVertex* pvin, Gaudi::SymMatrix3x3& hess,
                                         ROOT::Math::SVector<double, 3>& d0vec, double& dchi2wg,
                                         const LHCb::Track* vtrack, double wg, IGeometryInfo const& geometry ) const {
  ...
  if ( mstate.checkLocation( LHCb::State::Location::ClosestToBeam ) != true ) {
    ...
    if ( fabs( mstate.qOverP() ) > 0 ) {
      StatusCode sc = m_fullExtrapolator->propagate( mstate, pvin->position().z(), geometry );
      if ( sc.isFailure() ) return false;
    } else {
      StatusCode sc = m_linExtrapolator->propagate( mstate, pvin->position().z(), geometry );
      if ( sc.isFailure() ) return false;
    }
  } else {
    StatusCode sc = m_linExtrapolator->propagate( mstate, pvin->position().z(), geometry );
...
void PVOfflineRecalculate::print_stats() {
  ...
  int i = 0;
  for ( const auto& name : s_counter_name ) { printRat( name, m_counter_count[i++], m_counter_count[0] ); }
  info() << "--------------------------------------------------------------------------------" << endmsg;
  info() << endmsg;
}
