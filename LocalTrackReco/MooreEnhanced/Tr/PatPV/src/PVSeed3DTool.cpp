...
private:
  // squared variables to gain in speed
  ...
  Gaudi::Property<double> m_TrackPairMaxDistance{
      ...
      [=]( auto& ) { this->m_TrackPairMaxDistanceSq = std::pow( this->m_TrackPairMaxDistance, 2 ); },
  ...
  Gaudi::Property<double> m_zMaxSpread{this, "zMaxSpread", 3. * Gaudi::Units::mm,
                                       [=]( auto& ) { this->m_zMaxSpreadSq = std::pow( this->m_zMaxSpread, 2 ); },
                                       Gaudi::Details::Property::ImmediatelyInvokeHandler{true}};
};

namespace {
  constexpr double s_TrackPairMaxCos2Theta = 0.999 * 0.999;
  ...
  //=============================================================================
  bool xPointParameters( const LHCb::Track* track1, const LHCb::Track* track2, const double max_distanceSq,
                         const double max_cos2Theta, Gaudi::XYZPoint& closestPoint ) {
    ...
    return ( d12 * d12 / d1122 < max_cos2Theta ) && ( ( close1 - close2 ).Mag2() < max_distanceSq );
    // could also be evaluated as distancesq = v0.Mag2() + l1 * d10 - l2 * d20;
  }
  ...
  // weighedMean
  //=============================================================================
  seedPoint wMean( const std::vector<closeNode>& close_nodes, const seedTrack& base_track ) {
    seedPoint pseed;
    pseed.position.SetXYZ( 0., 0., 0. );
    pseed.error.SetXYZ( 0., 0., 0. );
    pseed.multiplicity = close_nodes.size();
    ...
    for ( const auto& cn : close_nodes ) {
      ...
      double c2 = cos2Theta( base_track.lbtrack->firstState().slopes(), cn.seed_track->lbtrack->firstState().slopes() );
...
DECLARE_COMPONENT( PVSeed3DTool )

//=============================================================================
// getSeeds
//=============================================================================
std::vector<Gaudi::XYZPoint> PVSeed3DTool::getSeeds( LHCb::span<const LHCb::Track* const> inputTracks,
    ...
    if ( !its1->used ) {
      ...
      for ( auto its2 = seed_tracks.begin(); its2 != seed_tracks.end(); its2++ ) {
        if ( !its2->used && its1 != its2 ) {
          ...
          if ( xPointParameters( lbtr1, lbtr2, m_TrackPairMaxDistanceSq, s_TrackPairMaxCos2Theta, closest_point ) ) {
      ...
      if ( msgLevel( MSG::DEBUG ) ) {
        ...
        for ( itd = close_nodes.begin(); itd != close_nodes.end(); itd++ ) {
          debug() << format( " xyz %7.3f %7.3f %7.3f ", itd->closest_point.X(), itd->closest_point.Y(),
                             itd->closest_point.Z() )
                  << endmsg;
        }
      }
  ...
  for ( unsigned int i = 0; i < seed_points.size(); i++ ) {
    ...
    if ( msgLevel( MSG::DEBUG ) ) {
      debug() << " xyz seed multi  " << seed_points[i].position.X() << " " << seed_points[i].position.Y() << " "
              << seed_points[i].position.Z() << " | " << seed_points[i].error.X() << " " << seed_points[i].error.Y()
              << " " << seed_points[i].error.Z() << " | " << seed_points[i].multiplicity << " " << endmsg;
    }
  }
  return seeds;
}
