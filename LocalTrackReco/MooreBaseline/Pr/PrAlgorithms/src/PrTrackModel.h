...
namespace LHCb::Pr {
  ...
  namespace Forward {
    ...
    struct VeloSeed {
      ...
      friend bool operator==( const VeloSeed& lhs, const VeloSeed& rhs ) {
        return ( lhs.x0 == rhs.x0 ) && ( lhs.y0 == rhs.y0 ) && ( lhs.tx == rhs.tx ) && ( lhs.ty == rhs.ty );
      }
    ...
    struct VeloSeedExtended {
      VeloSeedExtended( int iTrack, const VeloSeed& veloseed, const FTZoneCache::ZoneCache& cache )
        ...
        for ( auto iLayer{0}; iLayer < Detector::FT::NFTLayers; ++iLayer ) {
          if ( iLayer % 2 ) {
          ...
          const auto yStraightInZone = seed.y( cache.zoneZPos[iZone] );
      ...
      std::array<float, Detector::FT::NFTLayers>   betterZ;
      std::array<float, Detector::FT::NFTLayers>   xStraightInZone;
      std::array<float, Detector::FT::NFTUVLayers> yCorrTermLayer;
      std::array<float, Detector::FT::NFTUVLayers> yCorrTermAbsLayer;
    ...
    private:
      alignas( 64 ) std::array<int, Detector::FT::NFTXLayers * planeMulti> m_idxOnPlane{};
      alignas( 64 ) std::array<int, Detector::FT::NFTXLayers> m_planeSize{};
      alignas( 64 ) std::array<float, 5> m_s{nan, nan, nan, nan, nan};
      alignas( 64 ) std::array<float, Detector::FT::NFTXLayers> m_betterDz{};
      FitCoords<int, Detector::FT::NFTXLayers * planeMulti> m_coordsToFit{};
    ...
    private:
      FitCoords<int, Detector::FT::NFTUVLayers> m_coordsToFit{};
      TrackPars<4>                              m_xParams{{nan, nan, nan, nan}};
      TrackPars<3>                              m_yParams{{nan, nan, nan}};
      std::bitset<Detector::FT::NFTUVLayers>    m_usedPlanes{};
    ...
    public:
      PrForwardTrack( static_vector<int, Detector::FT::NFTXLayers * XCandidate::planeMulti> coordsToFit,
    ...
    private:
      FitCoords<int, Detector::FT::NFTLayers> m_coordsToFit{};
      TrackPars<4>                            m_xParams{{nan, nan, nan, nan}};
      TrackPars<3>                            m_yParams{{nan, nan, nan}};
      std::pair<float, float>                 m_chi2NDoF{nan, nan};
      float                                   m_quality{0.f};
