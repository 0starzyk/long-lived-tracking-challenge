...
public:
  ...
  LinParFit( size_t nparams )
      ...
      , m_rhs( nparams, FLT( 0 ) )
      , m_cov( ( nparams * ( nparams + 1 ) ) / 2, std::numeric_limits<FLT>::quiet_NaN() )
      , m_sol( nparams, std::numeric_limits<FLT>::quiet_NaN() )
      , m_npts( 0 )
      , m_nptsSolved( -1 ) {
    if ( 0 == nparams ) throw std::invalid_argument( "number of parameters must be positive" );
  ...
  LinParFit( const std::vector<std::string>& parnames )
      ...
      , m_cov( ( parnames.size() * ( parnames.size() + 1 ) ) / 2, std::numeric_limits<FLT>::quiet_NaN() )
      , m_sol( parnames.size(), std::numeric_limits<FLT>::quiet_NaN() )
      , m_names( parnames )
      , m_npts( 0 )
      , m_nptsSolved( -1 ) {
    if ( 0 == parnames.size() ) throw std::invalid_argument( "number of parameters must be positive" );
  }
  ...
  void accumulate( FLT wy, const FLTVECT& wgrad ) {
    ...
    for ( unsigned i = 0; i < m_rhs.size(); ++i ) {
      m_rhs[i] += wy * wgrad[i];
      for ( unsigned j = 0; j <= i; ++j ) m_mat[( i * ( i + 1 ) ) / 2 + j] += wgrad[i] * wgrad[j];
    }
    ++m_npts;
  }
   ...
   * @endcode
   */
  void accumulate( FLT y, FLT sigma, const std::vector<FLT>& x, const std::vector<size_t>& maxdeg,
                   size_t totmaxdeg = -1 ) {
    /// verify input arguments
    if ( x.empty() ) {
      throw std::invalid_argument( "need at least one variable in x" );
    } else if ( x.size() != maxdeg.size() ) {
      throw std::invalid_argument( "lengths of vectors x and maxdeg must match" );
    }
    // use a "multiindex" to enumerate all terms in polynomial
    m_multiidx.clear();
    m_multiidx.assign( maxdeg.size(), size_t( 0 ) );
    ...
    const auto next = [this, &maxdeg, totmaxdeg]() {
      ...
      while ( endidx != idx ) {
        ...
        if ( *max >= *idx && totmaxdeg >= std::accumulate( begidx, endidx, size_t( 0 ) ) ) { return true; }
    ...
    m_wgrad.clear();
    m_wgrad.reserve( m_rhs.size() );
    do {
      // q = x_0^multiidx[0] * x_1^multiidx[1] * ...
      const FLT q = std::inner_product(
          std::begin( x ), std::end( x ), std::begin( m_multiidx ), FLT( 1 ), []( FLT x, FLT y ) { return x * y; },
  ...
  operator bool() noexcept {
    if ( m_nptsSolved == m_npts )
      return !std::isnan( m_cov[0] );
    else
      return solve();
  }
  ...
  const std::vector<FLT>& solution() {
    if ( !solve() ) throw std::domain_error( "matrix not positive definite" );
    return m_sol;
  }
  ...
  FLT correl( unsigned i, unsigned j ) {
    ...
    if ( i < j ) std::swap( i, j );
    return m_cov.at( ( i * ( i + 1 ) ) / 2 + j ) /
           std::sqrt( m_cov.at( ( i * ( i + 1 ) ) / 2 + i ) * m_cov.at( ( j * ( j + 1 ) ) / 2 + j ) );
  }
  ...
  operator std::string() {
    bool               okay = solve();
    std::ostringstream tmpbuf;
    tmpbuf << "FIT RESULT: FIT " << ( okay ? "CONVERGED" : "FAILED, COVARIANCE MATRIX NOT POS. DEF." )
    ...
    tmpbuf << std::setw( 4 ) << std::right << "NR"
           ...
           << " " << std::setw( fltw ) << std::setprecision( fltp ) << std::scientific << std::right << "VALUE"
           << " +/- " << std::setw( fltw ) << std::setprecision( fltp ) << std::scientific << std::right << "ERROR"
           << std::endl;

    for ( unsigned i = 0; i != m_sol.size(); ++i ) {
      tmpbuf << std::setw( 4 ) << std::right << i << " " << std::setw( namew ) << std::left << m_names[i] << " "
             << std::setw( fltw ) << std::setprecision( fltp ) << std::scientific << std::right << m_sol[i] << " +/- "
             << std::setw( fltw ) << std::setprecision( fltp ) << std::scientific << std::right
             << std::sqrt( m_cov[( i * ( i + 1 ) ) / 2 + i] ) << std::endl;
    }
    tmpbuf << std::endl << "CORRELATION MATRIX" << std::endl << std::setw( 4 ) << "";
    for ( unsigned i = 0; i != m_sol.size(); ++i ) tmpbuf << " " << std::setw( 6 ) << i;
    tmpbuf << std::endl;
    for ( unsigned i = 0; i != m_sol.size(); ++i ) {
      tmpbuf << std::setw( 4 ) << i;
      for ( unsigned j = 0; j <= i; ++j ) {
        tmpbuf << " " << std::setw( 6 ) << std::setprecision( 3 ) << std::fixed << std::right
               << ( m_cov[( i * ( i + 1 ) ) / 2 + j] /
                    std::sqrt( m_cov[( i * ( i + 1 ) ) / 2 + i] * m_cov[( j * ( j + 1 ) ) / 2 + j] ) );
      }
      tmpbuf << std::endl;
    }
