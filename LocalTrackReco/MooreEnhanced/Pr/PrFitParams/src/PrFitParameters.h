 ...
 *  Class to store and compute/update parameters for the pattern recognition algorithms
 *
 *  @author Olivier Callot
 *  @date   03/07/2012
 */
class PrFitParameters final {
public:
  /// Standard constructor
  PrFitParameters() : m_fit( 1 ){};

  PrFitParameters( const std::string& title, const std::vector<double>& param ) : m_fit( 1 ) { init( title, param ); }
  ...
  friend StatusCode parse( PrFitParameters& p, std::string const& s ) {
    std::vector<double> vd;
    using Gaudi::Parsers::parse;
    return parse( vd, s ).andThen( [&] { p.init( p.m_title, vd ); } );
  }

  friend std::ostream& toStream( PrFitParameters const& p, std::ostream& os ) {
    using Gaudi::Utils::toStream;
    return toStream( p.m_par, os );
  }
