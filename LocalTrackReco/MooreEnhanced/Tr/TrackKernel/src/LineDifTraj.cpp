...
std::unique_ptr<Trajectory<double>> LineDifTraj::clone() const { return std::make_unique<LineDifTraj>( *this ); }

/// Constructor from the middle point and a direction vector
LineDifTraj::LineDifTraj( const Point& middle, const Vector& dir, const Range& range )
    : DifTraj<4>( range ), m_dir( dir.Unit() ), m_pos( middle ) {}

/// Constructor from a begin and an end point
LineDifTraj::LineDifTraj( const Point& begPoint, const Point& endPoint )
...
Trajectory<double>::Point LineDifTraj::position( double arclength ) const { return m_pos + arclength * m_dir; }

/// First derivative of the trajectory at arclength from the starting point
Trajectory<double>::Vector LineDifTraj::direction( double /* arclength*/ ) const { return m_dir; }

/// Second derivative of the trajectory at arclength from the starting point
Trajectory<double>::Vector LineDifTraj::curvature( double /* arclength */ ) const { return Vector( 0, 0, 0 ); }

/// Create a parabolic approximation to the trajectory
/// at arclength from the starting point
void LineDifTraj::expansion( double arclength, Point& p, Vector& dp, Vector& ddp ) const {
  ddp = Vector( 0, 0, 0 );
  dp  = m_dir;
...
double LineDifTraj::muEstimate( const Point& point ) const { return m_dir.Dot( point - m_pos ); }

// 1st order approx OK everywhere
double LineDifTraj::distTo1stError( double, double, int ) const { return 10 * Gaudi::Units::km; }
...
LineDifTraj::Derivative LineDifTraj::derivative( double s ) const {
...
std::ostream& LineDifTraj::print( std::ostream& out ) const {
