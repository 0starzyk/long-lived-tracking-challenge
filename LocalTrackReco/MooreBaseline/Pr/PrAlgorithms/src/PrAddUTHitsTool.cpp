...
namespace LHCb::Pr {
  ...
  private:
    ...
    Gaudi::Property<float> m_majAxProj{
        ...
        [=]( auto& ) { this->m_invMajAxProj2 = 1 / ( this->m_majAxProj * this->m_majAxProj ); },
  ...
  void PrAddUTHitsTool::addUTHits( LongTracks& tracks, const LHCb::UTDAQ::GeomCache& geomcache,
    ...
    if ( const auto size = tracks.size(); size % simd::size ) {
      ...
      tracks.simd()[size].field<TracksTag::States>( 0 ).setPosition( Vec3<F>{1.f, 1.f, 1.f} );
      tracks.simd()[size].field<TracksTag::States>( 0 ).setDirection( Vec3<F>{1.f, 1.f, 1.f} );
    }
    ...
    for ( const auto& fState : filteredStates.scalar() ) {
      ...
      if ( myUTHits.size() <= UTLayers ) continue;
  ...
  static_vector<int, MaxUTHits> PrAddUTHitsTool::findUTHits( const ProxyState&     fState,
    ...
    for ( unsigned itB{0}; itB + 2 < permutations.size(); ++itB ) {
      ...
      if ( nbPlane <= UTLayers || UTHits.size() > goodUT.size() ) continue; // -- Need at least hits in 3 planes
  ...
  bool PrAddUTHitsTool::selectHits( const ProxyState&                                 fState,
    ...
    constexpr const auto maxSectorsPerRegion = static_cast<int>( UTInfo::SectorNumbers::MaxSectorsPerRegion );
    constexpr const auto maxLayer            = totUTLayers;
    constexpr const auto maxRegion           = static_cast<int>( UTInfo::DetectorNumbers::Regions );
  ...
  float PrAddUTHitsTool::calculateChi2( float bestChi2, float p, unsigned bestSize, Container& goodUT,
    ...
    while ( goodUT.size() > 2 ) {
      ...
      if ( goodUT.size() > MaxUTHits || ( bestChi2 < totChi2 && goodUT.size() > bestSize ) ) {
